...
inline double calculateNorm(const ComplexVector& psi, double dx) {
    double norm_val = 0.0;
...
inline ComplexVector calculateRHS(const ComplexVector& psi, double dx, double g) {
    int N = psi.size();
    ...
    for (int i = 1; i < N-1 ; ++i) {
        ...
        // Combined right-hand side: -i(-d²ψ/dx² + g|ψ|²ψ)
        rhs[i] = -Complex(0.0, 1.0) * (-d2psi + nonlinear);
    }
...
inline ComplexVector rungeKutta4(const ComplexVector& psi, double dt, double dx, double g) {
    int N = psi.size();
...
// inline void gaussian(ComplexVector& psi, vector<double> x, double A, double sigma, double x0, double k0, int N, double dx) {
...
inline void plane_wave(ComplexVector& psi, vector<double> x, int N, int L, double dx) {
...
inline void wave_packet(ComplexVector& psi, vector<double> x, double u, int N, double dx) {
    
    for (int i = 0; i < N; ++i) {
        psi[i] = sqrt(2)*1.0/cosh(x[i])*exp(complex(0.0,1.0)*u*x[i]);
        // psi[i] = sqrt(2.0) / cosh(x[i]);
    }
...
inline void solitons(ComplexVector& psi, vector<double> x, double u, double theta, int N, int L, double dx) {
...
inline vector<double> initialise_grid(int N, int L, double dx){
    // Initialise grid
    vector<double> x(N);
    for (int i = 0; i < N; ++i) {
        x[i] = -L/2 + i * dx;
