...
inline void sigma_m(Matrix &sig, int i, int N){
    for (int k = 0; k<N; ++k){
        if (k==i){
...
inline void sigma_m_x(Matrix &sig, int i, int N){
    for (int k = 0; k<N; ++k){
        if (k==i){
...
inline void sigma_m_n_x(Matrix &sig, int m,int n, int N){
    for (int k = 0; k<N; ++k){
        if (k==m || k==n){
            sig = sig.tensorProduct(X);
        }
...
inline void sigma_m_m_plus_1(Matrix &sig, int i, int N){
    for (int k = 0; k<N; ++k){
        if (k==i||k==(i+1)%N){
            sig = sig.tensorProduct(X);
        }
...
inline Matrix XTerm(int N,double g){
    int size = std::pow(2, N);  // Size of the matrix (2^N)
...
inline std::vector<double> calculateSecondDerivative(const std::vector<double>& g_values, 
const std::vector<double>& function_values) {
    ...
    // Central difference formula for grid
...
inline double ellipticIntegralE(double x) {
...
inline double theoreticalGroundStateEnergy(double g) {
...
inline StateVector matrixVectorMultiply(const Matrix& H, const StateVector& psi) {
...
inline StateVector computeDerivative(const Matrix& H, const StateVector& psi) {
...
// Perform RK4 step
inline StateVector rk4Step(const Matrix& H, const StateVector& psi, double dt) {
    size_t size = psi.size();
...
inline void normalizeState(StateVector& psi) {
    double norm = 0.0;
...
inline double dotProduct(const StateVector& a, const StateVector& b) {
    if (a.size() != b.size()) {
...
inline double SZ(const StateVector psi,int N){
...
inline double SX(const StateVector psi,int N){
...
inline double CXX(const StateVector psi,int N){
